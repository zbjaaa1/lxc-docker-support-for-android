...
sed -i "$row3 i\                snprintf(name, CGROUP_FILE_NAME_MAX, \"%s.%s\", cfts->ss->name, cfts->name);" $RUNC_ADD_FILE
sed -i "$row3 i\        if (cfts->ss && (cgrp->root->flags & CGRP_ROOT_NOPREFIX) && !(cfts->flags & CFTYPE_NO_PREFIX)) {" $RUNC_ADD_FILE
